module.exports = {
    ...
    build(currentItemsCount, currentPage, pageSize, totalResource, basePath) {

        const data = {};
        data.current_items_count = currentItemsCount;
        data.total_items_count = totalResource;
        ...
        for(let i = 1; i <= data.number_of_pages; i++){
            pagis.push({
                number: i, 
                // link: `${basePath}?page=${i}&page_size=${data.requested_page_size}`,
                active: (i == data.current_page_number) ? true : false
            })
        }
        data.pagis = pagis
